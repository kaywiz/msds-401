...
```{r test1aiv}
pnorm(19.5, .25*100, sqrt(100*.25*.75))
pbinom(19, 100, .25)
```
...
p <- data.frame(Outcomes=seq(0,10), Probabilities=dpois(0:10, .05*100))
b <- data.frame(Outcomes=seq(0,10), Probabilities=dbinom(0:10, 100, .05))

p_plot <- ggplot(p, aes(x=Outcomes, y=Probabilities)) + 
  geom_bar(stat="identity", fill="#fd6468") +
  ggtitle("Poisson Probabilities") +
  theme(legend.position = "none")

b_plot <- ggplot(b, aes(x=Outcomes, y=Probabilities)) + 
  geom_bar(stat="identity", fill="#5b1a18") +
  ggtitle("Binomial Probabilities") +
  theme(legend.position = "none")

grid.arrange(p_plot, b_plot, nrow=1)
...
```{r test1ci}
a <- c(0, 1, 2, 3, 4, 5, 6)
b <- c(0.215, 0.230, 0.240, 0.182, 0.130, 0.003, 0.001)

exp <- round(sum(a*b), 2)
exp
var <- round(sum((a-exp)^2*b), 2)
var
```
...
```{r test1cii}
csum <- data.frame(Outcomes=a, Probs=cumsum(b))
cat("Median: (",median(csum$Outcomes),", ",median(csum$Probs),")")
ggplot(data = csum, aes(x = Outcomes, y = Probs)) + 
  geom_point(size = 2, color="#fd6468") + 
  ggtitle("Cumulative Probabilities") +
  ylab("Cumulative Probabilities") +
  geom_point(x=3, y=median(csum$Probs), shape=1, size=6, color="#5b1a18") +
  geom_text(x=4, y=.867, label="Median = (3, 0.867)", color="#5b1a18")
...
ggplot(faithful, aes(x=waiting)) +
  geom_histogram(size=.2, fill="#fd6468", color="#5b1a18", bins = 12) + 
  ggtitle("Waiting Times between Eruptions") +
...
erupt3 <- subset(wait70, wait70$eruptions < 3)
cond_prob <- nrow(erupt3)/nrow(wait70)
cond_prob
```
...
```{r test2ai}
wait_70_3 <- subset(faithful, faithful$waiting > 70 & faithful$eruptions < 3.0)
wait_70_3
ggplot(faithful, aes(x = waiting, y = eruptions)) + 
  geom_point(size = 1, color="#fd6468") + 
  ggtitle("Eruption Times | Waiting Times") +
  xlab("Waiting Times (minutes)") +
  ylab("Eruption Times (minutes)") +
  geom_hline(yintercept=3, color="#d57237", linetype=2) +
  geom_vline(xintercept=70, color="#d57237", linetype=2) +
  geom_point(x=71, y=2.383, size=1, color="#5b1a18") +
  geom_text(x=76, y=2.383, label="(71, 2.383)", color="#5b1a18")
```

(2)(a)(ii) (2 points) What does the plot suggest about the relationship between eruption time and waiting time?

***Answer: The plot suggests that there is a loose positive correlation between waiting times and eruption times -- that is, generally, the longer the waiting time between eruptions, the longer the duration of the eruption. However, the plot also suggests that this is a bimodal distribution, as there is a distinct cluster of shorter eruptions/waiting times and longer eruptions/waiting times.***
...
ggplot(pairs, aes(x = First, y = Second)) + 
  ...
  xlab("First Paired Time (minutes)") +
  ylab("Second Paired Time (minutes)")
```
...
boxplot(result1$weight, main="Weights | Diet 1", ylab="Weights (grams)", col="#fd6468")
boxplot(result3$weight, main="Weights | Diet 3", ylab="Weights (grams)", col="#5b1a18")
```
...
ggplot(diet_3, aes(x=weight20, y = weight21)) + 
  geom_point(size = 1, color="#fd6468") + 
  geom_abline(intercept=0, slope=1, color="#d57237", linetype=2) +
...
qqline(Nile)
boxplot(Nile, horizontal=FALSE, notch=TRUE, col="darkblue", main="Boxplot of Nile River Flows", ylab="Flow")
```
...
chi_sq_stat <- ((mar_tbl[1,1]-e11)^2/e11) + ((mar_tbl[1,2]-e12)^2/e12) + ((mar_tbl[2,1]-e21)^2/e21) + ((mar_tbl[2,2]-e22)^2/e22) + ((mar_tbl[3,1]-e31)^2/e31) + ((mar_tbl[3,2]-e32)^2/e32)
